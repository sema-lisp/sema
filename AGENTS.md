...
2. **Bump versions** in `Cargo.toml`: the workspace version *and* the 15 inter-crate `=X.Y.Z` pins (16 lines total). One-shot: `sed -i '' -e 's/^version = "OLD"/version = "NEW"/' -e 's/version = "=OLD"/version = "=NEW"/g' Cargo.toml`, then `grep -c 'NEW' Cargo.toml` (expect 16) and `grep -c 'OLD' Cargo.toml` (expect 0).
