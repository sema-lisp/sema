...
- **Adding an opcode**: *append* the `Op` variant — never insert (discriminants are positional and index `.semac` files). Then update every site that switches on the opcode: `from_u8`, `stack_effect`, `_assert_all_ops_covered`, the `op` module const (`opcodes.rs`); `advance_pc` + `stack_effect_operand` (`serialize.rs`); the two bytecode walks in `compiler.rs`; `op_name` + the decode arm (`disasm.rs`); the VM dispatch arm (`vm.rs`); the suspension-scan walk (`instruction_size` + the `scan_chunk` classification in `hof_sync.rs` — both exhaustive, so they fail to compile if missed); and the spec above. A missed pc-walk/decode site desyncs the disassembler silently — **verify with `jake smoke-bytecode`** (compile → disasm → run every example); unit tests won't catch it.
...
- **Builtin fn**: add to `crates/sema-stdlib/src/*.rs`, register in that module's `register()` fn, add an eval test, add a doc entry (see "Documentation coverage" below).
...
## Documentation coverage — a gate, not a checklist

Doc coverage is enforced by tests in both directions. Do not verify it by hand.

- **`jake docs-check` is the whole answer.** It runs strict generation (every
  entry has a summary), checks the committed index is current
  (`git diff --exit-code`), and runs `builtin_doc_coverage`, which proves every
  registered builtin and special form has an entry. It is part of `jake ci`.
- The reverse gate is `builtin_doc_names_are_all_registered`
  (`crates/sema-lsp/src/tests.rs`): every documented name must be a real binding.
  Entries for functions that do not exist ship phantom docs — LSP completion and
  REPL apropos then advertise names that throw "Unbound variable". This is the
  `memory/*` bug it pins.
- **Do not grep `crates/sema-docs/entries/` to check whether something is
  documented.** Run the gate. Its failure message lists the exact missing names.
- **Do not edit `crates/sema-docs/builtin_docs.generated.json`.** It is
  generated by `sema-docs gen`. Edit the entry under
  `crates/sema-docs/entries/stdlib/` or `entries/special-forms/`, then run
  `jake docs`. `docs-check` diffs the generated file, so a hand edit either gets
  overwritten by the next `jake docs` or fails the gate with a confusing diff.
- Add a builtin and its doc entry in the same change. The gate blocks the merge
  otherwise, so splitting them only costs a round trip.

## Changelog entries are files, not edits

`CHANGELOG.md` is assembled from `changelog.d/` at release time. **Never write a
new entry into it directly** — not with an editor tool, not with `sed`, not with
a one-off Python script. It is 3000+ lines, so inserting into it lands in the
wrong section or fails on an ambiguous match.

Correcting an entry is a different operation and follows the release boundary:

- **Not yet released** (the fragment is still in `changelog.d/`) — edit the
  fragment file. It is a few lines, so the edit is unambiguous.
- **Already released** (the entry is in a `## X.Y.Z` section) — edit
  `CHANGELOG.md` directly. The fragment is gone and assembly is one-way, so this
  is the only route. It is safe because you are rewording text you have already
  read, not inserting into a file you have not. `changelog-release` refuses to
  touch an existing version's section, so it will never overwrite your fix.

- To record a change, create `changelog.d/<type>-<slug>.md`, where type is one of
  `added changed deprecated removed fixed security`. The body is the markdown
  bullet. Write it in the curated prose style the existing sections use: what
  changed, why, and what it replaces. A commit subject is not enough.
- Preview the next section with `jake release.changelog-preview`; it writes nothing.
- Releases run `jake release.changelog-release version=X.Y.Z` (Release Procedure
  step 3).
- One file per user-visible change. Two branches adding entries touch different
  files, so a changelog conflict is not possible.
- Format details and examples: `changelog.d/README.md`.
...
1. **Run the full CI-equivalent suite locally** (plain `cargo nextest run` is NOT enough — it skips the example/bytecode smoke tests that run in CI): `jake ci` — it runs the exact recipes verify.yml runs (lint, shell hygiene, docs-check, workspace tests, LSP e2e, smokes, web-runtime freshness, packaged-crate boundary test) and all must pass. Skipping `jake examples`/`jake smoke-bytecode` locally is how a regression once shipped past four releases. The publish workflow (`publish-npm.yml`, both registries) `needs:` the `verify` gate (`.github/workflows/verify.yml`, the same definition ci.yml runs per-push) — a red suite blocks publishing. After pushing the tag, confirm `gh run list` shows CI **and** the publish gate green; never assume "started" == "passed". (The cargo-dist `Release`/binaries workflow is autogenerated and not test-gated — advisory, except its fail-hard web-runtime freshness step.)
...
3. **Assemble the changelog**: `jake release.changelog-release version=X.Y.Z` —
   prepends a `## X.Y.Z` section built from `changelog.d/` and deletes the
   fragments. Check it first with `jake release.changelog-preview`. Never
   hand-edit `CHANGELOG.md` (see "Changelog entries are files, not edits").
