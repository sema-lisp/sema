...
pub use async_signal::{
    ...
    set_llm_scope_task_callbacks, set_otel_task_callbacks, set_pending_native_outcome,
    set_resume_value, set_run_scheduler_callback, set_runtime_quantum, set_spawn_callback,
    set_task_reaped_callback, set_usage_scope_task_callbacks, set_yield_signal,
    take_debug_coop_resume, take_pending_native_outcome, take_resume_value, take_task_llm_scope,
    take_task_otel, take_task_usage_scope, take_yield_signal, BlockingSleepFn, CancelCallbackFn,
    DebugCoopResume, InterruptCallbackFn, IoHandle, IoPoll, LegacyChannel, LegacyPromise,
    LegacyRuntimeBridge, LlmScopeCaptureFn,
    LlmScopeInstallFn, LlmScopeTakeFn, OtelInstallFn, OtelScopeFn, OtelTakeFn, PromiseSetKind,
