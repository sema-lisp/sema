...
- `(agent/run agent message opts)` — returns a map `{:response ... :messages ... :session ... :usage ...}`, where `:messages` is the full conversation (so you can continue it), `:session` is a `Conversation` you can pass back in as `:session` on a later call, and `:usage` is this turn's own token/cost tally — `{:prompt-tokens :completion-tokens :total-tokens :cache-read-tokens :cache-creation-tokens :model :cost-usd :calls}` (`:cost-usd` is present only when pricing for the model is known; `:calls` is the number of billed provider round trips this turn made). Unlike `llm/last-usage` (most recent call only) or `llm/session-usage` (every call in the process, needs diffing to attribute one turn), `:usage` is exactly this turn's total, correctly summed across every round of the tool loop. `opts` accepts `:messages` (prior history to resume from), `:memory` (a `memory/open` handle: the thread's prior turns seed the conversation, and the run's new text turns are appended back — including the turns a cancelled run had produced), `:on-tool-call` (a callback fired for each tool call), `:on-text` (a callback fired with each streamed assistant text delta), and `:on-partial` (a callback fired with the partial `{:response :messages :session :usage}` map when a cooperative run ends by error or cancellation — the only way to recover the transcript of a cancelled turn, since `async/await` on a cancelled task raises). The blocking path does not call `:on-partial`. The tool callback receives an event map: `{:event "start" :tool name :args …}` before the tool runs and `{:event "end" :tool name :args … :result … :error bool :duration-ms n}` after. `:on-text` is called with each text chunk as the reply is generated, so a front-end can render the answer live; the streamed chunks concatenate to `:response`. Without `:on-text` the reply is returned whole (non-streaming). Tool calls work under streaming too. `:on-partial` runs in the cancelled task's last step, which cannot park again: it must only capture the value (`set!`), never suspend on I/O or `async/await`. If it raises, its error is ignored so it cannot replace the run's original error. It reports the rounds that completed; the text of a round still streaming when the cancel lands is delivered through `:on-text` only.
