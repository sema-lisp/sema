...
Watcher startup is asynchronous. If backend creation, path registration, or
event delivery fails, this call raises the backend error and retires the handle.
An empty result can mean that startup is still pending.
