...
impl Drop for Interpreter {
    fn drop(&mut self) {
        ...
        // thread reclaims the env. Host-resource teardown hooks are the
        // exception: releasing their ownership is non-evaluating and must not
        // depend on a later GC. The persistent runtime field is left in place;
        // its own `Drop` (`close_for_interpreter_drop`) cancels every task and
        // closes the inbox WITHOUT driving any VM quantum, so it is bounded
        // and panic-safe (no re-entrant evaluation while unwinding).
        ...
        // Host-resource leases belong to the interpreter lifetime, not the
        // reachability of its global env. Embedders may retain `global_env`, so
        // run each registry's weak teardown hook before clearing Value roots.
        let _ = self.ctx.try_run_interpreter_teardown_hooks();
...
impl Interpreter {
    ...
    ///
    /// Literal loads in a top-level form are scanned during file compilation so
    /// macros in those files are available to following forms. Scanning only
    /// registers macro definitions; the load stays in generated bytecode, so
    /// ordinary loaded definitions and side effects run only at runtime.
    pub fn compile_to_bytecode(&self, input: &str) -> Result<sema_vm::CompileResult, SemaError> {
        ...
        for expr in &exprs {
            self.expand_file_compile_form(expr, &mut expanded)?;
        }
    ...
    /// Expand one source-order top-level form for file compilation. `begin`
    /// and `progn` have no top-level scope, so processing each child separately
    /// makes a literal macro load visible only to later children.
    fn expand_file_compile_form(
        &self,
        expr: &Value,
        ...
        if let Some(items) = expr.as_list() {
            if items
                ...
                .is_some_and(|name| matches!(name.as_str(), "begin" | "progn"))
            {
                for child in &items[1..] {
                    self.expand_file_compile_form(child, expanded)?;
                }
                return Ok(());
            }
        }

        if self.ctx.current_file_dir().is_some() {
            preload_literal_load_macros(expr, &self.global_env, &self.ctx)?;
        }
        let expr = self.expand_for_vm(expr)?;
        if !expr.is_nil() {
            expanded.push(expr);
        }
        Ok(())
    ...
    /// Compile source with relative literal macro loads resolved from `path`.
    pub fn compile_file_to_bytecode(
        &self,
        path: &std::path::Path,
        input: &str,
    ) -> Result<sema_vm::CompileResult, SemaError> {
        let canonical = path
            .canonicalize()
            .map_err(|error| SemaError::eval(format!("compile {}: {error}", path.display())))?;
        self.ctx.push_file_path(canonical);
        let result = self.compile_to_bytecode(input);
        self.ctx.pop_file_path();
        result
    }
...
/// Collect every symbol in ordinary binding syntax. This includes `_` and `&`,
/// which are only markers in specific pattern contexts.
fn collect_symbols(value: &Value, out: &mut HashSet<Spur>) {
    if let Some(symbol) = value.as_symbol_spur() {
        out.insert(symbol);
        return;
    }
    match value.view() {
...
/// Collect bindings from a vector or map destructuring pattern.
fn collect_destructure_symbols(pattern: &Value, out: &mut HashSet<Spur>) {
    if let Some(symbol) = pattern.as_symbol_spur() {
        if resolve(symbol) != "_" {
            out.insert(symbol);
        }
        return;
    ...
    match pattern.view() {
        ValueView::Vector(items) => {
            for item in items.iter() {
                if item
                    .as_symbol_spur()
                    .is_some_and(|symbol| resolve(symbol) == "&")
                {
                    continue;
                }
                collect_destructure_symbols(item, out);
            }
        }
        ValueView::Map(map) => {
            let keys = Value::keyword("keys");
            if let Some(key_names) = map.get(&keys) {
                collect_destructure_symbols(key_names, out);
            }
            for (key, value) in map.iter() {
                if key != &keys {
                    collect_destructure_symbols(value, out);
                }
            }
        }
...
/// Collect bindings from a regular `let` target, which is either a plain symbol
/// or a vector/map destructuring pattern.
fn collect_binding_target_symbols(target: &Value, out: &mut HashSet<Spur>) {
    if target.as_vector().is_some() || target.as_map_ref().is_some() {
        collect_destructure_symbols(target, out);
    } else {
        collect_symbols(target, out);
    }
}

/// Collect bindings from a lambda or multiple-values formal list.
fn collect_formal_symbols(formals: &Value, out: &mut HashSet<Spur>) {
    let Some(items) = formals.as_list().or_else(|| formals.as_vector()) else {
        collect_symbols(formals, out);
        return;
    };
    for item in items {
        if item
        ...
        if item.as_vector().is_some() || item.as_map_ref().is_some() {
            collect_destructure_symbols(item, out);
        } else {
            collect_symbols(item, out);
        }
...
/// Collect bindings from a `define` target, including function-signature
/// shorthand and destructuring targets.
fn collect_define_symbols(target: &Value, out: &mut HashSet<Spur>) {
    if let Some(signature) = target.as_list() {
        if let Some(name) = signature.first() {
            collect_symbols(name, out);
        }
        for formal in &signature[1..] {
            if formal.as_vector().is_some() || formal.as_map_ref().is_some() {
                collect_destructure_symbols(formal, out);
            } else if formal
                .as_symbol_spur()
                .is_some_and(|symbol| resolve(symbol) != ".")
            {
                collect_symbols(formal, out);
            }
        }
    } else if target.as_vector().is_some() || target.as_map_ref().is_some() {
        collect_destructure_symbols(target, out);
    } else {
        collect_symbols(target, out);
    }
...
/// Collect bindings from a `match` pattern. Quoted lists and map keys are data,
/// while `_` and vector `&` are pattern markers.
fn collect_match_symbols(pattern: &Value, out: &mut HashSet<Spur>) {
    if let Some(symbol) = pattern.as_symbol_spur() {
        if resolve(symbol) != "_" {
            out.insert(symbol);
        }
    ...
    match pattern.view() {
        ...
        ValueView::Vector(items) => {
            for item in items.iter() {
                ...
                collect_match_symbols(item, out);
            }
        }
        ValueView::Map(map) => {
            let keys = Value::keyword("keys");
            if let Some(key_names) = map.get(&keys) {
                collect_match_symbols(key_names, out);
            }
            for (key, value) in map.iter() {
                if key != &keys {
                    collect_match_symbols(value, out);
                }
            }
        }
        _ => {}
...
fn collect_defined_names(expr: &Value, out: &mut HashSet<Spur>) {
    ...
    match resolve(head).as_str() {
        ...
        "define" | "defun" | "defn" | "defmulti" | "deftool" | "defagent" => {
        ...
        "define-values" => {
            if let Some(formals) = items.get(1) {
                collect_formal_symbols(formals, out);
            }
        }
        "define-record-type" => {
            // (define-record-type Name (ctor field...) pred (field accessor)...)
            // The type tag and field labels describe the record; the constructor,
            // predicate, and accessors are the names this form binds.
            if let Some(ctor_spec) = items.get(2).and_then(Value::as_list) {
                if let Some(ctor) = ctor_spec.first().and_then(Value::as_symbol_spur) {
                    out.insert(ctor);
                }
            }
            if let Some(predicate) = items.get(3).and_then(Value::as_symbol_spur) {
                out.insert(predicate);
            }
            for field_spec in &items[4..] {
                if let Some(accessor) = field_spec
                    .as_list()
                    .and_then(|spec| spec.get(1))
                    .and_then(Value::as_symbol_spur)
                {
                    out.insert(accessor);
                }
            }
...
/// Scan a literal top-level `load` without evaluating ordinary loaded code.
fn preload_literal_load_macros(
    expr: &Value,
    env: &Env,
    ctx: &EvalContext,
) -> Result<(), SemaError> {
    ...
    match name.as_str() {
        "load" if items.len() == 2 && items[1].as_str().is_some() => {
            preload_loaded_file_macros(&items[1..], env, ctx)
        }
        _ => Ok(()),
    }
}

/// Scan a loaded source file. Macro declarations in that file are available to
/// the importing source at compile time, while all other forms remain runtime
/// work in the original `load` form.
fn preload_loaded_source_macros(
    expr: &Value,
    env: &Env,
    ctx: &EvalContext,
) -> Result<(), SemaError> {
    ...
    match name.as_str() {
        ...
        "begin" | "progn" => {
            for child in &items[1..] {
                preload_loaded_source_macros(child, env, ctx)?;
            }
            Ok(())
        }
        "defmacro" => register_defmacro(items, env),
        "define-syntax" => register_define_syntax(items, env),
        _ => Ok(()),
    }
}

fn preload_loaded_file_macros(
    args: &[Value],
    env: &Env,
    ctx: &EvalContext,
) -> Result<(), SemaError> {
    let module = special_forms::prepare_load(args, ctx)?;
    let _load = ctx.enter_module_load(module.identity)?;

    if sema_vm::is_bytecode_file(&module.bytes) {
        return Ok(());
    }
    let source = String::from_utf8(module.bytes).map_err(|error| {
        SemaError::Io(format!("load {}: invalid UTF-8: {error}", module.requested))
    })?;
    let (exprs, spans) = sema_reader::read_many_with_spans(&source)?;
    ctx.merge_span_table(spans);
    ctx.push_file_path(module.file_path);
    let result = (|| {
        for expr in &exprs {
            preload_loaded_source_macros(expr, env, ctx)?;
        }
        Ok(())
    })();
    ctx.pop_file_path();
...
fn expand_macros_in(
        ...
        if !items.is_empty() {
            if let Some(s) = items.first().and_then(|v| v.as_symbol_spur()) {
                ...
                if name == "quasiquote" {
                    ...
                    for item in &items[1..] {
                        expanded.push(expand_quasiquoted(expander, env, item, shadow, 1)?);
                    }
                ...
                match name.as_str() {
                    "define" => {
                        return expand_binder_form(
                            expander,
                            env,
                            expr,
                            items,
                            shadow,
                            collect_define_symbols,
                        )
                    }
                    "defun" | "defn" => {
                        return expand_defun_form(expander, env, expr, items, shadow)
                    }
                    "fn" | "lambda" => {
                        return expand_binder_form(
                            expander,
                    ...
                    "define-record-type" => {
                        // Every operand is record-definition syntax. In particular,
                        // field labels are not expression positions and a same-named
                        // macro must not transform them.
                        return Ok(expr.clone());
                    }
                    _ => {}
                }
    ...
    match expr.view() {
        ...
        ValueView::Map(map) => {
            let mut changed = false;
            let source_entries =
                sema_reader::ordered_map_literal_entries(expr).unwrap_or_else(|| {
                    map.iter()
                        .map(|(key, value)| (key.clone(), value.clone()))
                        .collect()
                });
            let mut expanded = Vec::with_capacity(source_entries.len());
            for (key, value) in source_entries {
                ...
                expanded.push((expanded_key, expanded_value));
            }
            if changed {
                Ok(sema_reader::ordered_map_literal(expanded))
            } else {
                Ok(expr.clone())
            }
        }
...
/// Traverse quasiquoted data without treating ordinary lists as macro calls.
/// Only an unquote at the current quasiquote depth returns to normal expansion.
fn expand_quasiquoted(
    expander: &MacroExpander<'_>,
    env: &Env,
    expr: &Value,
    shadow: &Shadow,
    ...
    if let Some(items) = expr.as_list() {
        let head = items
            .first()
            .and_then(|value| value.as_symbol_spur())
            .map(resolve);
        let next_depth = match head.as_deref() {
            Some("quasiquote") => depth + 1,
            Some("unquote" | "unquote-splicing") if depth == 1 => {
                let mut expanded = vec![items[0].clone()];
                for item in &items[1..] {
                    expanded.push(expand_macros_in(expander, env, item, shadow)?);
                }
            ...
            Some("unquote" | "unquote-splicing") => depth - 1,
            _ => depth,
        };
        let expanded = items
            .iter()
            .map(|item| expand_quasiquoted(expander, env, item, shadow, next_depth))
            .collect::<Result<Vec<_>, _>>()?;
        return Ok(rebuilt_list(expr, items, expanded));
    }

    match expr.view() {
        ValueView::Vector(items) => Ok(Value::vector(
            items
                .iter()
                .map(|item| expand_quasiquoted(expander, env, item, shadow, depth))
                .collect::<Result<Vec<_>, _>>()?,
        )),
        ValueView::Map(map) => {
            let source_entries =
            ...
            Ok(sema_reader::ordered_map_literal(
                source_entries
                    .iter()
                    .map(|(key, value)| {
                        Ok((
                            expand_quasiquoted(expander, env, key, shadow, depth)?,
                            expand_quasiquoted(expander, env, value, shadow, depth)?,
                        ))
                    })
...
fn expand_binder_form(
    ...
    items: &[Value],
    shadow: &Shadow,
    collect: fn(&Value, &mut HashSet<Spur>),
) -> EvalResult {
...
/// `(defun name (params...) body...)` / `(defn ...)`: both the function name
/// and its parameters are binding positions, while only the body is expanded.
fn expand_defun_form(
    expander: &MacroExpander<'_>,
    env: &Env,
    expr: &Value,
    items: &[Value],
    ...
    let mut bound = HashSet::new();
    collect_symbols(&items[1], &mut bound);
    collect_formal_symbols(&items[2], &mut bound);
    let inner = shadow.child(bound);
    let mut expanded = items[..3].to_vec();
    expanded.extend(expand_body(expander, env, &items[3..], &inner)?);
    Ok(rebuilt_list(expr, items, expanded))
}
...
fn expand_let_form(
    ...
    let mut bound = HashSet::new();
    let collect_binding: fn(&Value, &mut HashSet<Spur>) =
        if matches!(form, "let-values" | "let*-values") {
            collect_formal_symbols
        } else {
            collect_binding_target_symbols
        };
    if named {
        collect_symbols(&items[1], &mut bound);
    }
...
fn expand_match_form(
    ...
    for clause in items.iter().skip(2) {
        ...
        let mut bound = HashSet::new();
        collect_match_symbols(&parts[0], &mut bound);
        let inner = shadow.child(bound);
...
fn start_runtime_module(
    ...
    let is_import = matches!(completion, RuntimeModuleCompletion::Import { .. });
    ...
    let scope = match RuntimeModuleScope::enter(context, &identity, file_path.clone(), true) {
        Ok(scope) => scope,
