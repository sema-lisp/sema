...
pub fn compile(
    ...
    let mut has_unknown_global_mutation = false;
    let mut compiler = match known_natives {
        Some(mut natives) => {
            ...
            for expr in exprs {
                scan_global_rebinds(expr, &mut |mutation| match mutation {
                    GlobalMutation::Bind(spur) => {
                        natives.remove(&spur);
                    }
                    GlobalMutation::Unknown => {
                        has_unknown_global_mutation = true;
                        natives.clear();
                    }
                });
            }
    ...
    for expr in exprs {
        scan_global_rebinds(expr, &mut |mutation| match mutation {
            ...
            GlobalMutation::Unknown => has_unknown_global_mutation = true,
        });
    }
    compiler.has_unknown_global_mutation = has_unknown_global_mutation;
...
/// `Define` nodes and `set!` targets that resolved as Global. It descends into
/// lambda, module, and binding bodies so every optimization sees a rebind at
/// any depth. `eval`, `load`, and `import` can install arbitrary bindings, so
/// they report an unknown mutation.
fn scan_global_rebinds(expr: &ResolvedExpr, f: &mut impl FnMut(GlobalMutation)) {
    use ResolvedExpr as E;
    match expr {
        E::Const(_) | E::Quote(_) | E::Var(_) => {}
        E::DefineRecordType {
            ctor_name,
            pred_name,
            field_specs,
        ...
        E::Eval(val) | E::Load(val) => {
            f(GlobalMutation::Unknown);
            scan_global_rebinds(val, f);
        }
        E::Throw(val)
        | E::Delay(val)
...
struct Compiler {
    ...
    /// `eval`, `load`, and `import` can rebind any global, so static dispatch
    /// is unsafe.
    has_unknown_global_mutation: bool,
...
impl Compiler {
    fn new() -> Self {
        Compiler {
            ...
            has_unknown_global_mutation: false,
    ...
    fn compile_if(
        ...
        // (re)defined in this program and no unknown global mutation from
        // `eval`, `load`, or `import`. Peeks through a span wrapper on the test
                ...
                if let ResolvedExpr::Var(vr) = func.as_ref() {
                    if let VarResolution::Global { spur } = vr.resolution {
                        if resolve_spur(spur) == "not"
                            && !self.redefined_globals.contains(&spur)
                            && !self.has_unknown_global_mutation
                        {
    ...
    fn compile_define(&mut self, spur: Spur, val: &ResolvedExpr) -> Result<(), SemaError> {
        // Recursive calls keep their global lookup because this closure can
        // outlive a later evaluation that rebinds the global name.
        self.compile_expr(val)?;
    ...
    fn compile_lambda(&mut self, def: &LambdaDef<VarRef>) -> Result<(), SemaError> {
        ...
        inner.has_unknown_global_mutation = self.has_unknown_global_mutation;
    ...
    fn try_compile_intrinsic(
        ...
        if self.has_unknown_global_mutation || self.redefined_globals.contains(&spur) {
            return Ok(false);
        }
    ...
    fn compile_make_list(&mut self, exprs: &[ResolvedExpr]) -> Result<(), SemaError> {
        ...
        self.stack_height -= exprs.len() as u16;
        Ok(())
    }
    ...
    fn compile_make_map(
        ...
        self.stack_height -= (pairs.len() * 2) as u16;
        Ok(())
    }
    ...
    fn compile_prompt(&mut self, entries: &[PromptEntry<VarRef>]) -> Result<(), SemaError> {
        ...
        for entry in entries {
            match entry {
                PromptEntry::RoleContent { role, parts } => {
                    ...
                    self.stack_height -= parts.len() as u16;
                    self.stack_height += 1;
        ...
        self.stack_height -= entries.len() as u16;
        self.stack_height += 1;
        self.emit.emit_op(Op::Call);
        self.emit.emit_u16(1);
        self.stack_height -= 2;
    ...
    fn emit_runtime_call(&mut self, name: &str, args: &[&ResolvedExpr]) -> Result<(), SemaError> {
        ...
        self.stack_height -= 1 + args.len() as u16;
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn test_compile_if_not_peephole_guarded_by_dynamic_eval() {
        // `eval` can replace `not` at runtime, so the test must remain a
        // generic call and keep the ordinary branch polarity.
        let result =
            compile_many_str("(eval '(define not (lambda (x) x))) (lambda (x) (if (not x) 1 2))");
        let if_fn = result
            .functions
            .iter()
            .find(|f| {
        ...
        assert!(
            !ops.contains(&Op::JumpIfTrue),
            "dynamic eval must not trigger the polarity peephole: {ops:?}"
        );
        assert!(
            ops.contains(&Op::CallGlobal),
            "the test must dispatch to the runtime `not` binding: {ops:?}"
        );
        assert!(!ops.contains(&Op::Not), "no Not intrinsic either: {ops:?}");
    }

    #[test]
    fn test_compile_if_not_peephole_guarded_by_load_and_import() {
        for source in [
            r#"(load "module.sema") (lambda (x) (if (not x) 1 2))"#,
            r#"(import "module.sema") (lambda (x) (if (not x) 1 2))"#,
        ] {
            let result = compile_many_str(source);
            let if_fn = result
                .functions
                .iter()
                .find(|f| {
            ...
            assert!(
                !ops.contains(&Op::JumpIfTrue),
                "unknown global mutation must not trigger the polarity peephole: {ops:?}"
            );
            assert!(
                ops.contains(&Op::CallGlobal),
                "the test must dispatch to the runtime `not` binding: {ops:?}"
    ...
    #[test]
    fn test_compile_generated_global_bindings_disable_intrinsics() {
        for source in [
            "(define-record-type rec (+ x) rec? (x rec-x)) (+ 1 2)",
            "(deftool + \"tool\" {} (fn () nil)) (+ 1 2)",
            "(defagent + {}) (+ 1 2)",
        ] {
            let result = compile_many_str(source);
            let ops = extract_ops(&result.chunk);
            assert!(
                !ops.contains(&Op::AddInt),
                "generated `+` binding must not emit AddInt: {ops:?}"
            );
            assert!(
                ops.contains(&Op::CallGlobal),
                "generated `+` binding must dispatch through its global: {ops:?}"
            );
        }
    }
    ...
    // --- Top-level recursive calls retain global lookup ---
    ...
    #[test]
    fn test_compile_define_nontail_self_call_keeps_global_lookup() {
        let result = compile_str("(define (f n) (if (< n 2) 1 (+ (f (- n 1)) n)))");
        let ops = extract_ops(&find_fn(&result, "f").chunk);
        assert!(
            ops.contains(&Op::CallGlobal),
            "non-tail recursive call must resolve through the global: {ops:?}"
        );
        assert!(
            !ops.contains(&Op::CallSelf),
            "top-level recursive call must not capture an old closure: {ops:?}"
        );
    }

    #[test]
    fn test_compile_define_tail_self_call_keeps_global_lookup() {
        ...
        assert!(
            ...
            "tail recursive call must keep ordinary tail dispatch: {ops:?}"
        );
        assert!(
            ops.contains(&Op::LoadGlobal) && !ops.contains(&Op::SelfTailCall),
            "tail recursive call must load the current global binding: {ops:?}"
        );
    }
    ...
    #[test]
    fn test_take_in_top_level_tail_recursive_define() {
        // Global tail dispatch evaluates arguments before replacing the frame,
        // so last-use moves remain valid.
        let result = compile_str("(define (walk n) (if (= n 0) 'done (walk (- n 1))))");
        let ops = extract_ops(&find_fn(&result, "walk").chunk);
        assert!(ops.contains(&Op::TailCall), "sanity: {ops:?}");
        assert!(
            ops.contains(&Op::TakeLocal),
    ...
    fn test_take_in_nontail_self_recursive_define() {
        ...
        let result = compile_str("(define (f n) (if (< n 2) 1 (+ (f (- n 1)) n)))");
        let ops = extract_ops(&find_fn(&result, "f").chunk);
        assert!(ops.contains(&Op::CallGlobal), "sanity: {ops:?}");
        assert!(
            ops.contains(&Op::TakeLocal),
            "the final read of n is a move: {ops:?}"
        );
