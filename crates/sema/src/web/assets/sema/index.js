...
export class SemaInterpreter {
     ...
     */
    loadArchive(bytes) {
        const result = this._inner.loadArchive(toUint8Array(bytes));
        if (!result.ok) {
     ...
     */
    runEntry(path) {
        return this._inner.runEntry(path);
    }
    /**
     * Execute an embedded archive entry path with async HTTP replay support.
     */
    async runEntryAsync(path) {
        return (await this._inner.runEntryAsync(path));
    }
    /**
    ...
    invokeCallback(callback, ...args) {
        const handle = typeof callback === "number"
            ? callback
    ...
    registerFunction(name, fn) {
        this._inner.registerFunction(name, (...args) => {
