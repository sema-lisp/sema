...
**Found 2026-07-15, during the promise-op structural-ABI migration (Step D2).** The promise ops (`async/spawn`, `async/await`, `async/all`/`race`/`timeout`, the predicates, `async/cancel`, `async/run`) are now **runtime-only**: they suspend structurally through the `NativeOutcome` ABI (`Suspend`/`Runtime`) and are driven by the unified cooperative runtime. Their legacy value-ABI (`func`) is a hard-error stub. The legacy debug scheduler (native DAP + WASM cooperative debugger), which stepped async tasks via the retired `YieldReason`/`scheduler.rs` path, therefore cannot execute promise ops: a *debugged* program (no runtime quantum installed) that hits `async/spawn`/`async/await`/… gets the "requires runtime invocation" stub. The affected tests are `#[ignore]`d with reason `"async debugging pending runtime cooperative-debug mode — see docs/deferred.md"`:
