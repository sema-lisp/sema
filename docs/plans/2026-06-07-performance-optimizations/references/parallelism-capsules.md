> Source: https://oxcaml.org/documentation/parallelism/02-capsules/
...
*This page introduces the [“expert” capsule API](https://github.com/janestreet/basement/blob/master/src/capsule.mli). A more ergonomic, work-in-progress API is available in [`Portable.Capsule`](https://github.com/janestreet/portable/blob/master/kernel/src/capsule_intf.ml).*
...
Passwords **do** cross contention, so they can be freely shared between `portable` functions. Naively, that would introduce races, but passwords are also always [`local`](../../stack-allocation/intro). That means we still can’t transfer a password across domains: fork/join requires `global` functions, which cannot capture a `local` password.
