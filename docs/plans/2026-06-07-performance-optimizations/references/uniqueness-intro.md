> Source: https://oxcaml.org/documentation/uniqueness/intro/
> Mirrored from oxcaml.org for offline reference.

# Introduction to Uniqueness

See also the full feature [reference](../reference) and [common pitfalls](../pitfalls). In this document, we use the new [syntax for modes](../../modes/syntax).
...
These modes form two mode axes: the *uniqueness* of a value is either `unique` or `aliased`, while the *affinity* of a value is `once` or `many`. Similar to [locality](../../stack-allocation/intro), uniqueness and affinity are deep properties. If a value is at mode `unique` then all of its children are also `unique`. If a value is `once` then all of the closures it contains are also at mode `once`.
...
For example, `int`s and other immediates can be used as `unique` and `many`, no matter their mode. For example:
