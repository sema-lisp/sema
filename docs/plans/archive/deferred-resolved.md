...
## SRV-1 — async `http/serve` + concurrent connection handlers — RESOLVED (2026-07-17)

**Resolved 2026-07-17 across six passes; verified again 2026-07-27** — all 67
tests in `http_serve_concurrent_test` / `http_serve_cancel_test` /
`server_suite` pass with zero `#[ignore]`s. `http/serve` runs a task per
connection, `ws/recv` parks cooperatively, and the fail-fast guard that
rejected `http/serve` inside `async/spawn` is deleted. The pass-by-pass
narrative below is kept verbatim as the design record — read it as history;
its interim "STILL DEFERRED" headings were true only at the dates they carry.
Live follow-up traps that outlived this entry are tracked as **SRV-TRAPS-1**
...
## L2 — Code-lens execution + `sema/evalResult` notification e2e — RESOLVED (2026-07-27)
...
**Resolved 2026-07-27.** `crates/sema-lsp/tests/e2e/test_run_top_level.py` (new,
7 tests) covers the half `test_code_lens.py` never did: it invokes
`workspace/executeCommand sema.runTopLevel` for real and awaits the
`sema/evalResult` notification, asserting `ok`, `value`, `kind`, `error`,
`stdout`, `range`, and `elapsedMs` against the `EvalResultParams` contract in
`crates/sema-lsp/src/handlers/command.rs`.

Beyond the three steps proposed above, it also pins behavior that was
previously untested and is easy to break:

- **Running form N evaluates forms 0..=N.** `(+ x 41)` resolves `x` from the
  preceding `(define x 1)`. Sending only the selected form reads like an
  optimization and would break every lens on a non-first form.
- **stdout is captured separately from `value`**, not merged into it.
- **An out-of-range `formIndex` is dropped silently** — no notification, no
  panic. A stale lens on an edited document must not take the server down;
  the test then runs a valid form to prove the server survived.
- **An unknown command is ignored** the same way, with the same survival check.

The notification is captured via a `@client.feature("sema/evalResult")` handler
feeding an `asyncio.Queue`, registered once per test in an `eval_results`
fixture — `executeCommand` returns before the eval finishes, so awaiting its
reply proves nothing. Assertions were mutation-checked: deliberately wrong
expected values fail all five substantive tests, so none passes vacuously.

**Found while landing this:** the local `.venv` under `tests/e2e/` had a stale
console-script shebang pointing at the pre-move `~/code/sema-lisp/...` path, so
