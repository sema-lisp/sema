...
| `llm-batch-enrichment.sema-nb` | Classify a dataset concurrently with the three guardrails: bounded pool, response cache (re-runs are free), fail-closed budget — plus a cost report | LLM key¹ |
| `research-agents.sema-nb` | Agents with tools → streaming → two concurrent conversations → mid-conversation cancellation → sessions | LLM key¹ |
...
¹ `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY`; without one, LLM
cells print "(skipped)" and the run still completes cleanly. Spend per full
run is well under a cent (tiny prompts, cheap models).
